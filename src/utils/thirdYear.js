...
export async function generateStudentCertificate(student) {
  ...
  const existingPdfBytes = await fetch("/templates/3rd year.pdf").then(res =>
    res.arrayBuffer()
  );
...
drawText(`${student.totalMaxMarks}`, 380, 117);
  ...
  drawText(`${student.secondYearTotalMarks}`, 380, 330); 
  drawText(`${student.secondYearPracticalObtained}`, 440, 330); 
  drawText(`${student.secondYearTheoryObtained}`, 485, 330); 
  drawText(`${student.secondYearTotalObtained}`, 520, 330);   // drawText(` ${student.totalPracticalObtained}`, 150, 396);
